...
def local_train_pfedgraph(args, round, nets_this_round, cluster_models, train_local_dls, val_local_dls, test_dl, data_distributions, best_val_acc_list, best_test_acc_list, benign_client_list):
    ...
    # pick round for this round
    for net_id, net in nets_this_round.items():
        
        # pick dataloader and data_distribution
        ...
        if net_id in benign_client_list:
            ...
            # accuracy consider label distribution or not
        ...
        # record evaluation loss for benign clients
        if net_id in benign_client_list:
            val_acc = compute_acc(net, val_local_dls[net_id])
            personalized_test_acc, generalized_test_acc = compute_local_test_accuracy(net, test_dl, data_distribution)
...
# load split dataset
# net_dataidx_map: the data idx for party i (i=0,1,...,n)
# traindata_cls_counts: n * k matrix: entry i,j show the number of samples in class j in party i
# data_distributions: normalized distribution on traindata_cls_counts
...
dw = []
# initialize local models and record weights
...
# initialize uniform graph (no self-connection)
...
for round in range(cfg["comm_round"]):
    ...
    # pick model and record the start
    nets_param_start = {k: copy.deepcopy(local_models[k]) for k in party_list_this_round}
    
    # train_local_dls, val_local_dls, test_dl: dataloaders 
